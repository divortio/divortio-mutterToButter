...
################################################################################
#
# Divortio Audio mutterToButter - Core Processor
#
# Version: 5.0 - Optimized Single-Pass Engine
#
# Description:
# This optimized version combines all audio processing steps (cleanup,
# mastering, and polish) into a single, dynamically-built FFmpeg filter chain.
# This improves performance by eliminating intermediate file I/O.
#
################################################################################
...
usage() {
  ...
  echo "Please use the main 'mutterToButter.sh' script."
  exit 1
}
...
if [ "$run_mastering" = true ]; then
  ...
  rms_level_db=$(grep "RMS_level" "$temp_stats_log_file" | awk -F'=' '{print $2}')
  rm "$temp_stats_log_file"
  if [ -n "$rms_level_db" ]; then
    if [ "$run_gate" = true ]; then
      gate_threshold_db=$(echo "$rms_level_db - 18" | bc -l)
      gate_threshold_linear=$(echo "10^($gate_threshold_db/20)" | bc -l)
      filter_chain+=("agate=threshold=$gate_threshold_linear")
      log_message "  - Mastering: Dynamic gate enabled."
    fi
    demud_threshold=$(echo "$rms_level_db + 3" | bc -l)
...
# 5. Join the filter chain array into a single, comma-separated string.
IFS=,
